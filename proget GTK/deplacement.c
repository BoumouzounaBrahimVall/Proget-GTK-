...
void ajouter_element( mov*fixe,Individu ind, coordonnees *C){
    ...
    printf("nameOf: %s",ind.etatInd.virus[0].nomVirus);
    printf("indSEx:%d",ind.genre);
    g_object_set_data((GObject *) fixe->elem->im, "info", fixe->elem);
    if((fixe->population-1)<3600){
        int i=0;
...
virusChoix (gpointer user_data)
{
    ...
    MonFrame *frame = init_Frame(NULL, walo2(), 0, 10, " PERSONALISER UN VIRUS", 1, walo());
    frame = creer_Frame(frame);
    Monlabel *label[4];
    label[0] = init_label("label1", "Virulance", NULL, walo2(), walo());
    label[1] = init_label("label2", "Distance de contamination", NULL, walo2(), walo()),
            label[2] = init_label("label3", "letalite", NULL, walo2(), walo());
    label[3] = init_label("label4", "contagion(%)", NULL, walo2(), walo());
    label[0] = creer_label(label[0]);
    label[1] = creer_label(label[1]);
    label[2] = creer_label(label[2]);
    label[3] = creer_label(label[3]);
    MonComboBox *Virus[4];
    Virus[0] = init_comboBox(NULL, "combo virulance", dim5, walo(), 1);
    ...
    Virus[1] = init_comboBox(NULL, "combo_distance", dim5, walo(), 1);
    Virus[1] = creer_comboBox(Virus[1]);
    for (int i = 1; i <= 5; i++)
        Virus[1] = ajouter_number_comboBox(Virus[1], i);

    Virus[2] = init_comboBox(NULL, "letalite", dim5, walo(), 1);
    ...
    Virus[3] = init_comboBox(NULL, "contagion", dim5, walo(), 1);
    Virus[3] = creer_comboBox(Virus[3]);
    for (int i = 1; i <= 10; i++)
        Virus[3] = ajouter_number_comboBox(Virus[3], i * 10);

    MonFrame *frameBox = init_Frame(NULL, walo2(), 0, 10, "Information sur le virus", 1, walo());
    frameBox = creer_Frame(frameBox);
    MonBox *box7 = Init_box(frameBox->frame.elem, NULL, 'v', 0, dimAdd(30, 160), 0, 10, NULL, walo());
    box7 = creer_Box(box7);
    ...
    dialogchoix = init_dialog(NULL, "choix_virus", NULL, 1, walo2(), NULL, walo());
    creer_dialog(dialogchoix);
    gtk_window_set_resizable((GtkWindow *) dialogchoix->dialog, FALSE);
    MonBouton *bouton[2];
    bouton[0] = init_Button(NULL, NULL, NULL, walo2(), " ", NULL, 1, 1, NULL, walo());
    bouton[1] = init_Button(NULL, NULL, NULL, walo2(), " ", NULL, 1, 1, NULL, walo());
    bouton[0]->button.elem = ajouter_button_dialog(dialogchoix, "          Anuller         ", 2);
    bouton[1]->button.elem = ajouter_button_dialog(dialogchoix, "          OK         ", 2);
    ...
    g_signal_connect_swapped(bouton[0]->button.elem, "clicked", G_CALLBACK(gtk_widget_destroy),
                             dialogchoix->dialog);
    g_signal_connect(bouton[1]->button.elem, "clicked", G_CALLBACK(virus_Info), L);
...
/// frame 2:choix du caracteristique
    ...
    bouton7=init_Button(NULL,NULL,NULL,walo2(),"Sauvegarder",NULL,1,0,&couleur3,walo());
    bouton7=creer_Bouton(bouton7);
    bouton8=init_Button(NULL,NULL,NULL,walo2(),"Retour",NULL,1,0,&couleur3,walo());
    bouton8=creer_Bouton(bouton8);
    MonFrame*frame2=init_Frame(NULL,walo2(),0,10," ",1,walo());
    frame2=creer_Frame(frame2);
///label  ajouter label au frame
    Monlabel*label4=init_label("label4","\tPouvez-vous choisir les caracteristiques d un individu?",frame2->frame.elem,walo2(),walo());
...
///ajouter frame au fixed
    ...
    Mongrid *grid= init_creer_Grid("grid",NULL,walo(),walo2(),&col1);
...
/// box qui contient les boutons...
    ...
    //ajouter_Box(boxGeneral,frameBox[1]->frame.elem,1,1,0,1);
...
///declarer les  labels et les comboboxs
    Monlabel*label5=init_label("label5","Sexe",NULL,walo2(),walo()),
            *label6=init_label("label6","Age",NULL,walo2(),walo()),
            *label7=init_label("label7","Vitesse",NULL,walo2(),walo()) ,
            *label8=init_label("label8","Etat",NULL,walo2(),walo()) ,
            *label9=init_label("label9","Poumons",NULL,walo2(),walo()) ,
            *label14=init_label("label14","Genetiquement",NULL,walo2(),walo()),
            *label15=init_label("label15","Tension arterielle",NULL,walo2(),walo()),
            *label16=init_label("label16","Diabetique",NULL,walo2(),walo()),
            *label17=init_label("label17","Cardiaque",NULL,walo2(),walo());
    ...
    MonComboBox*combo[7];
    combo[0]=init_comboBox(NULL,"combo_sexe",dim5,walo(),1);
    ...
    combo[1]=init_comboBox(NULL,"combo_age",dim5,walo(),1);
    ...
    MonComboBox *Virus=init_comboBox(NULL,"combo_age",dim5,walo(),1);
    ...
    combo[2]=init_comboBox(NULL,"genetiquement",dim5,walo(),1);
    ...
    combo[3]=init_comboBox(NULL,"Tension",dim5,walo(),1);
    ...
    combo[4]=init_comboBox(NULL,"Diabetique",dim5,walo(),1);
    ...
    combo[5]=init_comboBox(NULL,"Cardiaque",dim5,walo(),1);
    ...
    combo[6]=init_comboBox(NULL,"Poumon",dim5,walo(),1);
            ...
            if(element){
                ...
                for(int i=0;i<4;i++){
                    ...
                    if(indiv->ind.etatInd.sein=='S'){
                        indiv->ind.etatInd.virus[0]=virs;break;
                    }
...
gint deplacer(mov*fixe )
{
    ...
    while (L)
    {
        ...
        if(L->ind.vivant<10)
            ...
            if(L->ind.etatInd.sein!='S'){
                for(int i=0;i<4;i++){
                    if(L->ind.etatInd.virus[i].id==0) break;
                    Infecter(L->x,L->y,L->ind.etatInd.virus[i],fixe);
                }
...
gboolean sauvegardage(char *text,mov *inter)
{
    ...
    if(tmp==NULL) fprintf(Mon_fichier,"1\n");
        ...
        while (tmp) {
            ...
            if (tmp->ind.etatInd.sein == 'S') fprintf(Mon_fichier, "%d\n", 1);
            else {
...
//draw_rect(cairo_t *cr,GtkWidget *widget)
{
...
void deplacementEspace(GtkWidget*but,gpointer data){
   ...
   /// gtk_grid_set_row_spacing(GTK_GRID(inter->grid), 5);
  ///  gtk_grid_set_column_spacing(GTK_GRID(inter->grid), 5);
    dimension D={24,24};
    ...
    for (i = 0; i < Nbcase; i++)
    {
        for (j = 0; j < Nbcase; j++)
        {
            ...
            //gtk_widget_add_events(inter->B[i][j]->eventbox.elem, GDK_BU);
            g_signal_connect(inter->B[i][j]->eventbox.elem, "button_press_event", G_CALLBACK(play), (gpointer *)inter);
            g_signal_connect(inter->B[i][j]->eventbox.elem, "button_press_event", G_CALLBACK(play), (gpointer *)inter);
        }
    }

    dimension dim={700,600};
    GtkWidget *bogen= gtk_box_new(GTK_ORIENTATION_VERTICAL,0);
    gtk_widget_set_size_request(bogen,600,700);

    GtkWidget *boxv= gtk_box_new(GTK_ORIENTATION_HORIZONTAL,25);//
    MonBouton *update= init_Button(NULL,"./images/play.png",NULL,walo2(),NULL,NULL,1,0,NULL,walo());
    creer_Bouton(update);
    gtk_button_set_relief((GtkButton *)update->button.elem, GTK_RELIEF_NONE);
    gtk_box_pack_start(GTK_BOX(boxv),update->button.elem,0,0,0);
    btn=gtk_button_new_from_icon_name("media-floppy",GTK_ICON_SIZE_DND);
    gtk_button_set_relief((GtkButton *)btn, GTK_RELIEF_NONE);
    gtk_box_pack_start(GTK_BOX(boxv),btn,0,0,0);
    ...
    gtk_box_pack_start(GTK_BOX(boxv),statis,0,0,0);
    ...
    gtk_box_pack_start(GTK_BOX(boxv),exit,0,0,0);
    inter->lablePop= gtk_label_new("\t\t\t\t\t\t\t\t\t\t\tPopulation:0 ");
    gtk_box_pack_start(GTK_BOX(boxv),inter->lablePop,0,0,10);
    gtk_box_pack_start(GTK_BOX(bogen),boxv,0,0,0);

    GtkWidget *boxscrol= gtk_box_new(GTK_ORIENTATION_VERTICAL,10);

    //box general
    scroll= init_Scroll(boxscrol,inter->grid,0,dim);
    creer_Scroll(scroll);
    gtk_box_pack_start(GTK_BOX(bogen),boxscrol,TRUE,TRUE,0);
    gtk_container_add(GTK_CONTAINER(F->window),bogen);
